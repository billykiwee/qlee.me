...
export default function Edit() {
  ...
  const Link = UserLinks.filter(
    (data) => data.user === User.email && data.id === LinkID
  ).map((link) => link)[0];
      ...
      {Link && (
        <div className="grid gap-2rem m-t-1" key={Link.id}>
          <div className="grid blocks gap-1rem">
              ...
              <div className="display align-top">
                <div className="grid gap-1rem justfy-s-b border-r-1 border p-1 white w-100p">
                      ...
                      <div className="grid">
                        ...
                        <div className="display gap">
                          <a
                            ...
                            className="f-s-20 link hover-link"
                          >
                            {Link?.shortLink}
                          </a>
                          <CopyClip link={Link} />
                  ...
                  <nav className="display justify-c wrap gap">
                    <div className="grid gap">
                      <button
                        className="grey h-3 w-3 border-r-2 p-lr-1 display gap hover"
                        onClick={(e) => setNavSelected("edit")}
                        style={
                          navSelected === "edit"
                            ? {
                    ...
                    <div className="grid gap">
                      <button
                        className="grey h-3 w-3 border-r-2 p-lr-1 display gap hover"
                        onClick={(e) => setNavSelected("qr-code")}
                        style={
                          navSelected === "qr-code"
                            ? {
                                background: "var(--blue-secondary)",
            ...
            {navSelected === "qr-code" ? (
              <QrCodeSection Link={Link} setQrCode={setQrCode} />
            ) : (
              <div className="grid gap-1rem">
                <div className="grid gap-1rem">
                  <div className="grid gap-1rem white shadow border-r-1 border p-2">
                    <div className="grid gap w-100p">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          seteditLink({ ...editLink, name: e.target.value })
                        }
                      />
                  ...
                  <div className="grid gap white shadow border-r-1 border p-2">
                    ...
                    >
                      <div className="display">
                        <label
                          htmlFor="active_views"
                          className="display gap-1rem click"
                        >
                          <SwitchInput
                            dimension={0.8}
                            checked={Link.linkInBio ? true : false}
                            onChange={(e) =>
                              addtoLinkInBio(
                                e.target.checked,
                                LinkID,
                                User,
                                UserLinks.filter((e) => e.linkInBio),
                                Link
                              )
                            }
                            id="active_views"
                ...
                <div className="grid gap-1rem">
                  ...
                  <div className="display justify-c">
                    <div
                      className="display justify-c gap click red-secondary p-1 h-1 border-r-1"
                      onClick={(e) =>
                        DeleteLink({
                          Stats,
                          link: Link,
...
export function GoToPricing({ children }) {
  return (
    <Redirect
      to="/pricing"
      className="display justify-c yellow border-r-100 hover-yellow p-04 h-1 w-1 hover-yellow"
      style={{ width: !children ? "1rem" : "" }}
    >
      <div className="display justify-c gap">
        {children}
        <div className="display click">
          <div className="display justify-c">
            <span className="display">
              <img src="/images/lock-solid.svg" width={14} alt="" />
            </span>
          </div>
        </div>
      </div>
