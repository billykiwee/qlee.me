...
export default function LinkIcon({ upload }) {
  ...
  async function uploadPhoto(fileInput) {
    try {
      ...
      db.collection("links").doc(Link.id).update({ icon: downloadUrl });
  ...
  return (
    <div className="display justify-c">
      <div className="edit-image-link">
        ...
        <label
          htmlFor="upload-img"
          className=" display justify-c border-r-100 white shadow border hover-white absolute click p-04 w-1 h-1"
        >
          <PencilSquareIcon width={16} />
          <input type="file" hidden id="upload-img" onChange={uploadPhoto} />
        </label>
      </div>
    </div>
  );
