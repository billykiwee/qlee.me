              ...
              >
                <div className="display gap-1rem">
                  <Link to={"/edit/" + link.id} className="display">
                    <img
                      src={getFavicon(link)}
                      className="w-2 h-2 border-r-100"
                      alt=""
                    />
                  </Link>
                  <div className="grid ">
      ...
      {links.length > linkListed && (
        <div className="display">
          <button
            ...
            onClick={(e) => links.length > 10 && add(linkListed + 10)}
          >
            <span className="f-s-16 c-black">
              Afficher + (
              {(links.length < 10 ? links.length : linkListed) +
                " liens sur " +
                links.length}
              )
            </span>
        ...
        {links.length < isUserPremium(User).max_links ? (
          <div className="display gap">
            ...
            <small className="c-grey f-w-300">
              Il te reste {isUserPremium(User).max_links - links.length}{" "}
              {isUserPremium(User).max_links - links.length > 1
                ? "disponibles"
                : "disponible"}
            </small>
          </div>
          ...
          <div className="grid gap-04">
            <div className="display gap">
              <InformationCircleIcon width={20} className="c-black" />
              <small className="c-red f-w-300">
                Tu dois upgrade ton compte
              </small>
            </div>
