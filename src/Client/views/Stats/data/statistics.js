...
export const statistics = (LinkStat) => {
  ...
  const localisation = LinkStat.map(
    (e) => e.adress?.country_code + "__" + e.adress?.country
  );
  const reference = LinkStat.filter((e) => e.reference !== "").map(
    (e) => e.reference && new URL(e.reference).origin
  );
