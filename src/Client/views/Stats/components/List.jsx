  ...
  return (
    <div className="grid gap">
      <div className="grid gap" id="div-links">
        {!linksFilters().length ? (
          <small className="text-align-c">Aucun résultat</small>
        ) : (
          linksFilters()
            .map((link) => {
              return (
                <div
                  className="display justify-s-b"
                  id={"link-" + link.id}
                  key={link.id}
                >
                  <Link
                          ...
                          <div className="grid gap">
                            <div className="display gap-04">
                              ...
                              <div className="display">
                                <div
                                  className="display disable green absolute border-r-04 p-04"
                                  id={"link-" + link.id}
                                >
                                  <small>Copié</small>
                                </div>
                  ...
                  <div className="display p-04">
                    <div
                      className="display w-2 h-2 justify-c click hover border-r-100"
                      onClick={(e) =>
                        DeleteLink({
                          link,
                          setMsg,
        ...
        {linksFilters().length > linkListed && (
          <div className="display">
            <button
              className="white h-4 p-1 border-r-1 shadow border"
              onClick={(e) => add(linkListed + 10)}
            >
              <span className="f-s-16 c-black">
                Afficher + ({linkListed + " liens sur " + linksFilters().length}
                )
              </span>
            </button>
          </div>
