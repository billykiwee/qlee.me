        ...
        <div className="grid gap-1rem">
          ...
          {UserLinks === "no links" ? (
            ...
            UserLinks.filter((isUserAuth) => isUserAuth.user === User.email)
              ...
              .map((topLink) => {
                return (
                  <div
                    className="grid gap-2rem justfy-s-b border-r-2 border border-b p-1 white"
                    key={topLink.id}
                  >
          ...
          {isUserPremium(User).plan === "FREE" && (
            <div className="display justify-c">
              <Link
                className="display justify-c gap yellow hover-yellow border-r-2 h-2 p-1"
                to="/pricing"
              >
                <LockClosedIcon width={14} color="black" />
                <small style={{ color: "black" }} className="text-align-c">
                  Upgrade ton compte pour voir toutes les statistiques
                </small>
              </Link>
            </div>
          )}
