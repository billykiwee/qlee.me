...
export default function CheckoutForm({ props }) {
  const { user, MSG, error, planID, billingID, total } = props;
  ...
  return (
    <div className="grid white border border-r-1 p-1 shadow gap-1rem">
      <div className="grid m-b-1 gap">
        ...
        <span className="c-grey f-w-300">
          Entre tes informations de contact et de paiement
        </span>
      </div>
      ...
      <div className="grid">
        ...
        <input
          type="text"
          className="div-input grey h-4 m-t-04"
          id="name"
          value={user?.name}
        />
      </div>
      <div className="grid">
        ...
        <input
          type="email"
          className="div-input h-4 grey m-t-04"
          id="email"
          value={user?.email}
        />
      </div>
      ...
      <div className="grid gap-1rem">
        <div className="grid gap">
          ...
          <div
            className="display justify-s-b border border-r-04 p-1 div-input grey h-4"
            id="card_number"
          >
          ...
          {error.id === "number" && (
            <small className="c-red">{error.message}</small>
          )}
        </div>
          ...
          <div className="display justify-s-b gap">
            <div className="display gap justify-s-b w-100p align-top">
              <div className="grid w-100p gap">
                ...
                <div
                  className="display justify-s-b border border-r-04 p-1 div-input h-4 grey"
                  id="card_expiry"
                >
                  <div className=" w-100p">
      ...
      {MSG.loader ? (
        ...
        <div className="display w-100p">
          <div className="display w-100p">
            <button
              className="blue c-white hover-blue border-r-1 f-s-16 h-4 p-1"
              type="submit"
            >
              <span>
                Payer {formatCurrency(total)}
                {billingID === "yearly" ? " pour 1 an" : "par mois"}
              </span>
            </button>
          </div>
        </div>
